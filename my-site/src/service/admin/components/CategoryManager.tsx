// 카테고리 관리 공통 컴포넌트 — 외부/내부 카테고리 두 페이지가 repository, title 만
// 다르고 UI 와 로직이 동일해서 prop 으로 흡수.
...
export default function CategoryManager({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await repository.reorder(newCategories.map((c) => c.id));
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setAdding(true);
    try {
      await repository.createCategory(newName.trim());
      setNewName('');
      await fetchCategories();
    } catch (error) {
  ...
  const handleRename = async (id: string) => {
    ...
    setSavingId(id);
    try {
      await repository.updateName(id, editingName.trim());
      setEditingId(null);
      await fetchCategories();
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-3xl font-extrabold text-blue-900 dark:text-blue-300 mb-6 tracking-tight">{title}</h1>
