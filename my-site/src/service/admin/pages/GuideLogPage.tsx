...
export default function GuideLogPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <h1 className="text-3xl font-extrabold text-blue-900 mb-2 tracking-tight">수정 이력</h1>
      <p className="text-gray-500 mb-6">
        {isDeleted ? (
          <span className="text-red-500">삭제된 글</span>
        ) : (
          guideTitle
        )}
      </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left px-4 py-3 font-bold text-gray-600">작업</th>
                  <th className="text-left px-4 py-3 font-bold text-gray-600">작업자</th>
                  <th className="text-left px-4 py-3 font-bold text-gray-600">일시</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log, idx) => (
                  <tr key={log.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-4 py-2.5 font-medium text-gray-800">{log.editorNickname}</td>
