...
export default function EligmaCalcPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-3xl font-extrabold text-blue-900 mb-6 tracking-tight">엘리그마 계산기</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* 성급 선택 */}
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">현재 성급</label>
            <select
              value={currentStar}
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">목표 성급</label>
            <select
              value={targetStar}
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">현재 보유 엘레프(조각)</label>
            <input
              type="number"
              min="0"
              value={ownedEleph}
          ...
          {/* 엘레프 1개당 가격 선택 */}
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">엘레프 1개당 가격</label>
            <select
              value={pricePerEleph}
              onChange={(e) => {
          ...
          {pricePerEleph < 5 && (
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">해당 가격 잔여 구매 횟수</label>
              <input
                type="number"
                min="0"
                max="20"
      ...
      {/* 결과 화면 */}
      {result && (
        <div className="rounded-xl shadow-sm border p-6 transition-all bg-white border-blue-200">
          <h2 className="text-xl font-bold mb-4 text-blue-900">
            계산 결과
          </h2>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm">
              <span className="block text-sm text-gray-500 mb-1">상점에서 구매할 조각 수</span>
              <span className="text-2xl font-bold text-gray-800">{result.neededEleph} 개</span>
            </div>
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 shadow-sm">
              <span className="block text-sm text-blue-600 font-bold mb-1">필요한 총 엘리그마</span>
              <span className="text-2xl font-bold text-blue-700">{result.eligmaCost} 개</span>
            </div>
          </div>
        </div>
      )}
