...
| **PR#9 — 인연랭크(Bond) 통합** | [data/planner/bond_exp.json](my-site/src/data/planner/bond_exp.json), [tables/bondExp.ts](my-site/src/service/planner/utils/tables/bondExp.ts), [BondTargetInput.tsx](my-site/src/service/planner/components/BondTargetInput.tsx), `cultivationCalculator.ts` 의 `calculateBondExp` / `favorMultiplier` / `calculateBondGifts` / `aggregateAllWithBond`, `DeficitPanel` 의 "인연 (권장 선물)" 섹션, [PLAN_bond_rank.md](PLAN_bond_rank.md) | ✅ (2026-05-17) |

**전체 9/9 완료 — 단, 커밋 분할 + RLS·E2E 검증 + 일부 추가 기능(성급업·정렬 DnD 일부) 미반영.**
...
| ~~호감도 (Bond)~~ | ✅ **PR#9 완료** | [PLAN_bond_rank.md](PLAN_bond_rank.md) 참고. `FavorItemTags` + `FavorItemUniqueTags` + config 의 `CommonFavorItemTags` union 매칭, `gear-favor` 그룹과 인벤토리 공용, `MaterialBreakdown` 으로 출처 분해 |
